/*
* NOTICE: This version of the Gamer.cpp file has been modified by Matthew Petry (fireTwoOneNine)
* To improve usability and decrease RAM usage while maintaining 100% compatibility with existing sketches.
*
* All rights to this source code remain with Tech Will Save Us.
*/
 ...
 */
void Gamer::printString(String string, int delaytime = 70)
{
  byte screen[8]={0};
  clear();
  for( int index = 0; index<string.length(); index++ ){
    ...
    while( (col = pgm_read_byte(&allLetters[letIx][colIx])) != LETEND ){
      appendColumn(screen, col, delaytime);
      colIx++;
    }
  }
  for( int i=0; i<8; i++)
    appendColumn(screen, 0, delaytime);
}
 ...
 */
void Gamer::appendColumn(byte* screen, byte col, int delaytime = 70)
{
  for( int i=0; i<8; i++){
    screen[i]<<=1;
...
const PROGMEM uint8_t Gamer::allLetters[85][9] = {
